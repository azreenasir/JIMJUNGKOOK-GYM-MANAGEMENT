...
<%-- 
    Document   : report
    Created on : Jul 10, 2022, 11:17:16 PM
    Author     : azree
--%>
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            <sql:query var="result" dataSource="${myDatasource}">
                SELECT count(paymentid) as pay, payment.packageid, package.packagetitle from payment join package on payment.packageid = package.packageid group by payment.packageid, package.packagetitle
            </sql:query>

                var data = google.visualization.arrayToDataTable([
                    ['Task', 'Hours per Day'],
            <c:forEach var="table" items="${result.rows}">
                    ['<c:out value="${table.packagetitle}" />', <c:out value="${table.pay}" />],
            </c:forEach>
                ]);
