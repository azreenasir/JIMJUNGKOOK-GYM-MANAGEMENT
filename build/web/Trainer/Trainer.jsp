<%-- 
    Document   : Client
    Created on : Jul 5, 2022, 10:48:00 PM
    Author     : azree
--%>
...
<html>
    ...
    <body>
        ...
        <sql:query var="result" dataSource="${myDatasource}">
            SELECT TRAINERID, FIRSTNAME, LASTNAME, DOB, PHONENO, GENDER, EMAIL, TRAINER.SHIFTID, SHIFT.SHIFTTITLE FROM TRAINER JOIN SHIFT ON TRAINER.SHIFTID=SHIFT.SHIFTID
        </sql:query>
        <div class="container">
            <div class="d-flex justify-content-between m-4">
                <div>
                    <h3>List of Trainer</h3>
                </div>
                <div>
                    <a href="AddTrainer.jsp" class="btn btn-primary">Add New Trainer</a>
                </div>
            </div>
            <hr>
                            ...
                            <table class="table table-dark table-striped table-hover">
                                ...
                                <tr>
                                    <c:forEach var="columnName" items="${result.columnNames}">
                                        <c:if test="${!columnName.equals('TRAINERID') && !columnName.equals('SHIFTID')}">
                                ...
                                <c:forEach var="row" items="${result.rowsByIndex}">
                                    <tr>
                                        <c:forEach var="column" items="${row}">
                                            <c:if test="${column != row[0]&&column != row[7]}">
                                        ...
                                        <td>
                                            ...
                                            <form action='../deleteTrainer.Servlet' method='POST'>
                                                <input type="hidden" name="id" value="${row[0]}">
